defmodule SmsWeb.StoreLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
        <.input field={@form[:storeID]} type="number" label="Store ID" />
  ...
  def handle_event("validate", %{"store" => store_params}, socket) do
    changeset =
      ...
      |> StoreManager.change_store(store_params)
  ...
  defp save_store(socket, :edit, store_params) do
    case StoreManager.update_store(socket.assigns.store, store_params) do
      {:ok, store} ->
  ...
  defp save_store(socket, :new, store_params) do
    case StoreManager.create_store(store_params) do
      {:ok, store} ->
        notify_parent({:saved, store})
